      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-x-auto">
        <table className="min-w-[1000px] w-full text-sm">
          ...
          <tbody>
            {loading && (
              <tr>
                <td colSpan={7} className="px-5 py-6 text-center">Loading orders...</td>
              </tr>
            )}
            {!loading && filteredOrders.length === 0 && (
              <tr>
                <td colSpan={7} className="px-5 py-6 text-center text-brownSoft">No orders found.</td>
              </tr>
            )}
            {paginatedOrders.map((order) => (
              <tr key={order.order_id} className="border-t border-slate-100 hover:bg-slate-50 transition">
                ...
                <td className="px-5 py-4">
                  <div>
                    <span className={`px-2 py-1 rounded text-xs font-medium ${getPaymentBadge(order.payment_status || 'pending')}`}>
                      {order.payment_status || 'pending'}
                    </span>
                    <p className="text-xs text-brownSoft mt-1">{order.payment_type || 'N/A'}</p>
                  </div>
                </td>
