      ...
      <div className="min-h-screen bg-white">
        ...
        <div className="flex items-center justify-center h-screen">
          <div className="text-center">
            <div className="inline-block animate-spin rounded-full h-12 w-12 border-b-2 border-[#7B5DE8]"></div>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="text-center mb-12">
            ...
            <p className="text-gray-600 text-lg">
              View and manage your software license orders
            </p>
          </div>
          ...
          {orders.length === 0 ? (
            <div className="bg-white rounded-2xl shadow-lg p-12 text-center">
              ...
              <button
                onClick={() => navigate("/")}
                className="bg-[#7B5DE8] text-white px-8 py-3 rounded-full hover:bg-[#6A4BC4] transition-all"
              >
                Browse Software
              </button>
            </div>
            ...
            <div className="space-y-6">
              {orders.map((order) => (
                ...
                >
                  ...
                  {/* Order Items */}
                  <div className="p-6">
                    <h3 className="font-bold text-lg text-[#482072] mb-4">Order Items</h3>
                          ...
                          <div key={item.id} className="border border-gray-200 rounded-lg p-4">
                            <div className="flex justify-between items-start mb-3">
                              <div className="flex-1">
                                ...
                                <p className="text-sm text-gray-600">Plan: {item.plan_name}</p>
                                <p className="text-sm text-gray-600 capitalize">Duration: {item.duration_type}</p>
                                <p className="text-sm text-gray-600">Quantity: {item.quantity}</p>
                                <p className="text-sm text-gray-600">
                                  Valid: {formatDate(item.issued_at)} - {formatDate(item.expires_at)}
                                </p>
                              </div>
                              <div className="text-right">
                                <p className="font-bold text-[#7B5DE8] text-lg">
                                  Rs. {parseFloat(item.unit_price).toLocaleString()}
                                </p>
                                <p className="text-xs text-gray-500">per license</p>
                              </div>
                            </div>
                                ...
                                {serialNumbers.map((sn, idx) => (
                                  <div key={idx} className="flex items-center gap-2 mb-1">
                                    ...
                                    <button
                                      ...
                                      className="text-[#7B5DE8] hover:text-[#6A4BC4] text-sm font-semibold"
                                    >
                                      Copy
                                    </button>
                                  </div>
